...
def evaluate(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            ...
            y_pred = model(node_features, adj_matrix)
...
def test(test_dataframe, psepos_path):
    ...
    for model_name in sorted(os.listdir(Model_Path)):
        print(model_name)
        model = MGUPPIS(LAYER, INPUT_DIM, HIDDEN_DIM, NUM_CLASSES, DROPOUT)
...
def main():
    ...
    print("Evaluate MGUPPIS on Test_60")
    test_one_dataset(Test_60, Test60_psepos_Path)

    print("Evaluate MGUPPIS on Test_315-28")
    test_one_dataset(Test_315_28, Test315_28_psepos_Path)

    print("Evaluate MGUPPIS on UBtest_31-6")
